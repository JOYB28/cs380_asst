...
static float g_objXScale = 1.0;
static float g_objYScale = 1.0;
...
struct ShaderState {
  ...
  // Scale
  GLfloat h_uVertexXScale;
  GLfloat h_uVertexYScale;
  ...
  ShaderState(const char* vsfn, const char* fsfn) {
    ...
    h_uVertexXScale = safe_glGetUniformLocation(h, "uVertexXScale");
    h_uVertexYScale = safe_glGetUniformLocation(h, "uVertexYScale");
...
static void display(void) {
  ...
  if (g_height > g_width) {
    g_objXScale = 1.0;
    g_objYScale = (g_width * 1.0) / g_height;
  } else if (g_height < g_width) {
    g_objXScale = (g_height * 1.0) / g_width;
    g_objYScale = 1.0;
  }

  cout << "g_objScale: " << g_objScale << endl;
  cout << "g_objXScale: " << g_objXScale << endl;
  cout << "g_objYScale: " << g_objYScale << endl;

  safe_glUniform1i(curSS.h_uTexUnit0, 0);
  safe_glUniform1i(curSS.h_uTexUnit1, 1);
  safe_glUniform1f(curSS.h_uVertexScale, g_objScale);
  safe_glUniform1f(curSS.h_uVertexXScale, g_objXScale);
  safe_glUniform1f(curSS.h_uVertexYScale, g_objYScale);
